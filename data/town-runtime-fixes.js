(function () {
  ...
  function bindDeveloperButton() {
    ...
    function activate(e) {
      ...
      lastActivation = now;
      if (e) { e.preventDefault(); e.stopPropagation(); }
      if (typeof window.toggleDebugMode === 'function') window.toggleDebugMode();
    }
  ...
  // 明示的に編集した trigger.area を正本として扱う。
  ...
  function applyCommonSignAssets() {
    ...
    function removeDecor(scene, matcher) {
      ...
      scene.decor = scene.decor.filter(function (item) { return !matcher(item || {}); });
    }
  ...
  // 横丁の既存パーツは、確定済みの表示サイズだけ runtime で補正する。
  ...
  // レジャーセンターの配置自体は data/town-maps.js が正本。
  // ここには「追加候補カタログ」だけを残す。
  ...
  if (window.visualViewport) window.visualViewport.addEventListener('resize', applyCameraZoom);
})();
