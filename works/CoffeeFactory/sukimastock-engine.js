...
(function (root) {
  ...
  const audio = {
    ...
    unlock() {
      ...
      this.unlocked = true;
      // Apps that provide no HTMLAudio pools do not need an extra AudioContext
      // just to satisfy the global input unlock path.
  ...
  const app = {
    ...
    replace(name, payload, options) {
      ...
      transition.easing = opts.easing || "smooth";
    ...
    draw() {
      ...
      if (transition.active) {
        ...
        const eased = (EASING[transition.easing] || EASING.smooth)(raw);
        const alpha = transition.phase === "out" ? eased : 1 - eased;
        root.noStroke();
