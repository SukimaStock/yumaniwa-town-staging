...
const MITER = "MITER";
  ...
  function popStyle() {
    ...
    C.fillStyle = saved.fillStyle;
    C.strokeStyle = saved.strokeStyle;
    ...
    C.spriteMode = saved.spriteMode;
    C.blendMode = saved.blendMode;

    if (C.ctx) {
      C.ctx.globalCompositeOperation =
        C.blendMode === ADDITIVE ? "lighter" : "source-over";
    }
  }

  // Codea compatibility: this project uses textMode(CENTER) to request
  // centered text. Canvas exposes this as text alignment, so map it here.
  ...
  function spriteMode(mode) {
    if (mode === CENTER || String(mode).toUpperCase() === "CENTER") {
      C.spriteMode = CENTER;
    } else {
      C.spriteMode = CORNER;
    }
  ...
  function normalizeBlendMode(mode) {
    ...
    return value === ADDITIVE ? ADDITIVE : NORMAL;
  }

  function blendMode(mode) {
    if (arguments.length === 0) return C.blendMode;
    C.blendMode = normalizeBlendMode(mode);
    if (C.ctx) {
      C.ctx.globalCompositeOperation =
        C.blendMode === ADDITIVE ? "lighter" : "source-over";
    }
  ...
  class CodeaImage {
    constructor(src) {
      ...
      this.element.onload = () => {
        this.width = this.element.naturalWidth || this.element.width || 0;
        this.height = this.element.naturalHeight || this.element.height || 0;
        this.loaded = this.width > 0 && this.height > 0;
      };
      this.element.onerror = () => {
        this.error = new Error("Failed to load image: " + this.src);
        this.loaded = false;
      };
  ...
  function readImage(src) {
    if (src instanceof CodeaImage) return src;
    if (src instanceof HTMLImageElement) {
      const wrapped = Object.create(CodeaImage.prototype);
      wrapped.src = src.currentSrc || src.src || "";
      wrapped.element = src;
      wrapped.width = src.naturalWidth || src.width || 0;
      wrapped.height = src.naturalHeight || src.height || 0;
      wrapped.loaded = src.complete && wrapped.width > 0 && wrapped.height > 0;
      wrapped.error = null;
      return wrapped;
    }

    const key = String(src);
    if (!C.imageCache.has(key)) {
      C.imageCache.set(key, new CodeaImage(key));
    }
    return C.imageCache.get(key);
  }
  ...
  function sprite(img, x, y, w, h) {
    const source = img instanceof CodeaImage ? img : readImage(img);
    if (!source || !source.loaded || !source.element) return false;

    let drawW = Number(w);
    let drawH = Number(h);

    if (!Number.isFinite(drawW)) drawW = source.width;
    if (!Number.isFinite(drawH)) {
      drawH = (Number.isFinite(Number(w)) && source.width > 0)
        ? drawW * (source.height / source.width)
        : source.height;
    }
    ...
    // The runtime uses a bottom-left, y-up coordinate system.
    // HTML images would therefore appear vertically mirrored unless we
    // flip only the sprite back to normal screen orientation.
    ctx.translate(x, y);
    ctx.scale(1, -1);

    if (C.spriteMode === CENTER) {
      ctx.drawImage(source.element, -drawW / 2, -drawH / 2, drawW, drawH);
    } else {
      // CORNER follows Codea's lower-left anchor in the y-up world.
      ctx.drawImage(source.element, 0, -drawH, drawW, drawH);
    }
